...
Use the live GitHub Pages build (once Pages is enabled for this repo):

```text
https://jt5d.github.io/xrai/
```
...
```bash
git clone https://github.com/JT5D/xrai.git
cd xrai/agent
node src/cli.js web
```
...
```bash
claude mcp add --transport stdio --scope project xrai -- node "$PWD/agent/src/cli.js" mcp
claude mcp get xrai
```

From inside `xrai/agent`:
